...
namespace StartupManager.Helpers.Tables {
    public class Table<T> {
        ...
        public void OutputToConsoleColored() {
            ...
            var lines = RawTableString.Split(TableParser.LineSeperator, StringSplitOptions.RemoveEmptyEntries);
            for (int lineIndex = 0; lineIndex < lines.Length; lineIndex++) {
                var line = lines[lineIndex];
                if (headerNames.All(x => line.Contains(x))) { //Header line skip
                    System.Console.WriteLine(line.Replace(TableParser.HeaderSeperator, string.Empty));
                    System.Console.WriteLine();
                    continue;
                }

                var model = Models[lineIndex - 1];
                var content = line.Split(TableParser.HeaderSeperator, StringSplitOptions.RemoveEmptyEntries);
